...
def start_server():
    ...
    while True:
        ...
        # Recevoir et afficher le message du client
        msg = receive_message(client_socket)
        print("Message reçu du client:", msg)

        # Fermer la connexion avec le client
        client_socket.close()

        # Si le message est vide, arrêter le serveur
        if msg is None:
            break
...
# Attendre un court instant pour que le serveur soit démarré
time.sleep(1)

# Se connecter au serveur en tant que client
client = connect_to_serv()

# Créer un dictionnaire de message à envoyer
message = {"action": "update", "data": {"temperature": 25, "humidity": 50}}
